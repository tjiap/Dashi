...
public class DBConnection {
	...
	private void executeUpdateStatement(String query) {
		...
		try {
			...
			stmt.executeUpdate(query);// 写操作，不返回值 可执行UPDATE,INSERT
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private ResultSet executeFetchStatement(String query) {
		...
		try {
			Statement stmt = conn.createStatement();
			System.out.println("\nDBConnection executing query:\n" + query);
			return stmt.executeQuery(query);// 读操作，要返回值 如SELECT
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	// Search restaurants is in logic level, so we don't want to include this
	// function in API package
	public JSONArray SearchRestaurants(String userId, double lat, double lon) {
		try {
			YelpAPI api = new YelpAPI();// Connect to yelp API
			JSONObject response = new JSONObject(api.searchForBusinessesByLocation(lat, lon));
			JSONArray array = (JSONArray) response.get("businesses");// only
																		// care
																		// the
																		// bussiness
																		// field
			...
			// read all the features get from SearchReataurants and convert them
			// to local variables
